...
// from (Prisma today, SQL-DDL later).
//
...
//
// The model defines the entire OLTP surface Phase 2 audits, even what a given
// parser cannot express: a Prisma parse fills Tables/Columns/Indexes/ForeignKeys
// and leaves Views/Procedures/Triggers empty (the SQL-DDL parser of slice 3 fills
// those). Every element carries an origin db.Pos{File, Line} so a finding and the
// baseline can anchor by file+line/content.
package db
