...
// TestOwnedCategories_CoversEveryDWRule replaces S1's
// TestOwnedCategories_EmptyInS1 (and S2's TestOwnedCategories_CoversEveryS2Rule,
// renamed for the same reason as TestAll_RegistersTheDWFamily above). Every DW
// rule emits exactly one surface category, and every one of those categories
// MUST be declared owned — an undeclared category can never be baselined or
// pruned (ADR 0019), which is the single way to corrupt an existing baseline.
func TestOwnedCategories_CoversEveryDWRule(t *testing.T) {
	want := []string{
