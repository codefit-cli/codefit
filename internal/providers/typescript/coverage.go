		...
		Deterministic: append([]coverage.Entry{
			{
				...
				Detail: "Hardcoded secrets: a variable whose NAME looks like a credential assigned a static string literal. SHAPES REACHED: const, let, var, a TYPE-ANNOTATED const or let, and an OBJECT-LITERAL PROPERTY at any position inside an object of any size — the object shape uses the scoped ellipsis and needed an engine change, because a census across two real TypeScript projects measured every object holding a credential-named string property at arity 5, 5, 5 and 3, four of four multi-property and NONE at arity 1. DECLARED SHAPE LIMIT, now the only one left and the largest still open: a CLASS FIELD is SILENT. The same census counted 316 class-field string assignments, about 10x the const shape. It is NOT the arity problem the ellipsis solved, and that was measured: the pattern class $C { $NAME = $VALUE } finds zero even in a class with a SINGLE field, because a rule cannot address the public_field_definition node on its own. Closing it needs a different mechanism, not another alternative. NAME MATCHING: the credential name is matched by name COMPONENT through the shared namematch vocabulary, the same words and the same matcher Go uses, so tokenizer, secretariat and passwordless are correctly SILENT. It used to be a raw substring, which affirmed all three at confidence 1.0. DECLARED NAME LIMIT, now shared with Go rather than divergent from it: an ALL-LOWERCASE CONCATENATION carries no boundary to tokenize on, so secretkey is silent (secretKey, secret_key and SECRET_KEY all fire). That single spelling used to fire in TypeScript through the substring matcher; losing it is a declared narrowing, accepted because the same matcher was what affirmed tokenizer as a credential. All of it pinned by TestShapeCensus.",
			},
			{
				ID:     "ts.weak-crypto",
			...
			},
			{
				...
				Claim:  "SQL injection built directly in the database call — a query assembled inline by string concatenation or by an interpolated template literal, such as db.query(`SELECT ... ${userInput}`). Recognized entry points: .query(), .execute(), Prisma $queryRawUnsafe / $executeRawUnsafe, and Knex .raw(). Assembly through an intermediate variable is surface (below), not here.",
				Detail: "SQL injection built directly in the database call, assembled inline by concatenation or by an interpolated template. Assembly through an intermediate variable is surface, not this rule. RECOGNIZED METHOD VOCABULARY: .query(), .execute(), Prisma's $queryRawUnsafe and $executeRawUnsafe, and Knex's .raw(). DECLARED LIMIT, still SILENT: better-sqlite3's .prepare(). This is a name limit, not a shape limit — the interpolated template IS matched; the method name is what the rule does not recognize. DELIBERATELY NOT MATCHED: Prisma's TAGGED $queryRaw / $executeRaw. The tagged form PARAMETERISES its interpolations and is the correct way to write raw SQL, so firing on it would be a false positive at confidence 1.0. A census of a real Prisma project measured the tagged form 3 times and the unsafe forms zero times, so a careless widening would have produced only false affirmations. The two forms parse to the same node type and differ only in the second child (arguments vs template_string). All of it pinned by TestShapeCensus.",
			},
			{
				...
				Detail: "Cross-site scripting through React dangerouslySetInnerHTML when the __html value is built inline, by concatenation or by an interpolated template. A plain-variable __html is surface; a constant __html is not flagged. THE ARITY LIMIT IS CLOSED: this rule used to reach __html only when it was the object sole property, so an object also carrying className was SILENT, and that limit was never declared anywhere — the canonical JSX form is a one-property object, so the common case worked by coincidence and the gap stayed invisible. The scoped object ellipsis now reaches __html at any position in an object of any size, and being zero-or-more it still matches the one-property form. Pinned by TestShapeCensus.",
			},
